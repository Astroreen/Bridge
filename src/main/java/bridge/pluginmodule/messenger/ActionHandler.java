...
public class ActionHandler implements Runnable {
    ...
    private void BridgeHandler(final @NotNull Action action, final ConcurrentLinkedQueue<String> data) {
        switch (action) {
            case RUN_UPDATER -> Bukkit.getPluginManager().callEvent(new RunUpdaterEvent());
        }
    }

    private void BungeeCordHandler(final @NotNull Action action, final ConcurrentLinkedQueue<String> data){
        switch (action){
