package bridge.modules.updater;

import bridge.exceptions.BridgeRuntimeException;
...
public class UpdateDownloader {
     ...
     * @throws BridgeRuntimeException Is thrown if there was any exception during the download process.
     */
    public void downloadToFile(final URL url) throws BridgeRuntimeException {
        ...
        try {
            ...
            if (tempFile.exists() && !tempFile.delete()) {
                throw new BridgeRuntimeException(prefix + "There is a broken file at '" + getRelativePath(tempFile)
                        + "'. Delete this file otherwise a new download is not possible." + suffix, e);
            }
            throw new BridgeRuntimeException(prefix + "The updater could not download the file!"
    ...
    private void checkAndCreateFile() throws BridgeRuntimeException {
        try {
            if (!tempFile.createNewFile()) {
                throw new BridgeRuntimeException("The file '" + getRelativePath(tempFile) + "' already exists!"
                        + " Please wait for the active download to finish."
                        + " If there is no active download delete the file manually.");
            }
        } catch (final IOException e) {
            throw new BridgeRuntimeException("The updater could not create the file '" + getRelativePath(tempFile)
                    + "'! Reason: " + e.getMessage(), e);
        }
    }

    private void checkAndCreateFolder(final @NotNull File file) throws BridgeRuntimeException {
        if (!file.exists() && !file.mkdirs()) {
            throw new BridgeRuntimeException("The updater could not create the folder '" + getRelativePath(file) + "'!");
        }
    }
    ...
    private String getRelativePath(final @NotNull File file) {
